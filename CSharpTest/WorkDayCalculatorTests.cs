...
namespace CSharpTest
{
    ...
    public class WorkDayCalculatorTests
    {
        ...
        public void TestWeekendBeforeStart() {
            ...
            WeekEnd[] weekends = new WeekEnd[1]
            {
                new WeekEnd(new DateTime(2023, 3, 3), new DateTime(2023, 3, 5))
            };

            DateTime result = new WorkDayCalculator().Calculate(startDate, count, weekends);

            Assert.IsTrue(result.Equals(new DateTime(2023, 3, 16)));
        }

        [TestMethod]
        public void TestMonthChange()
        {
            DateTime startDate = new DateTime(2023, 3, 22);
            int count = 12;
            WeekEnd[] weekends = new WeekEnd[2]
            {
                new WeekEnd(new DateTime(2023, 3, 25), new DateTime(2023, 3, 26)),
            new WeekEnd(new DateTime(2023, 4, 1), new DateTime(2023, 4, 2))
            };

            DateTime result = new WorkDayCalculator().Calculate(startDate, count, weekends);

            Assert.IsTrue(result.Equals(new DateTime(2023, 4, 6)));
        }
    }
}
